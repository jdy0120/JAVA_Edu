...
// 정보저장용 클래스들(Friend, HighFriend, UnivFriend)을 이용하여 실제 작업을 하는 클래스
	...
	// 이름을 입력받아 검색한 후 결과를 출력하는 메소드
		...
		boolean isNotFound = true;	// 검색결과 유무를 저장할 변수

		for (int i = 0 ; i < numOfFriends ; i++) {
			if (name.equals(friends[i].getName())) {
			// friends배열 안에 있는 인스턴스의 name값이 입력받은 검색어와 같으면
				friends[i].showData();
				isNotFound = false;
			}
		}

		if (isNotFound)	// 찾는 이름이 없을 경우
...
// 친구 이름으로 검색하여 검색된 친구의 모든 정보를 출력
// 검색결과가 없으면 '존재하지 않는 이름입니다.' 출력
// 두 명이상일 경우 검색된 모든 친구들을 출력
